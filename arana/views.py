...
def category(request, id, slug):
    ...
    caritem = Product.objects.filter(typee_id = id) 
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if register.is_valid():
            ...
            newuser.pix = pix
            newuser.save()
            messages.success(request, f'dear {user.username} your account is created succesfull')
            return redirect('signin')
        else:
...
def profile_update(request):
     ...
     if request.method == 'POST':
         ...
         if pform.is_valid():
             ...
             new = user.first_name.title()
             messages.error(request, f'dear {new} your profile updated generated the following error: {pform.errorrs}')
...
def password_update(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, f'{new} there is an error trying to change your passsword')
...
def add_to_cart(request):
    if request.method == 'POST':
        ...
        if cart:
            basket = Cart.objects.filter(user__username = request.user.username, paid=False, car_id=main.id, quantity=quantity).first()
            if basket:
...
def pay(request):
    if request.method == 'POST':
        api_key = 'sk_test_9ca2c2c99c45016589edc5c346a028fca8249cc6'
